...
class RGCNAggregator_global(nn.Module):
    ...
    def forward(self, t_list, ent_embeds, graph_dict, reverse):
        ...
        embed_seq_tensor = torch.zeros(len(len_non_zero), self.seq_len, self.h_dim).to(_device)
...
class RGCNAggregator(nn.Module):
    ...
    def forward(self, s_hist, s, r, ent_embeds, rel_embeds, graph_dict, global_emb, reverse):
        ...
        if length == 0:
            ...
            if g is None:
                ...
                s_embed_seq_tensor = torch.zeros(len(s_len_non_zero), self.seq_len, 4 * self.h_dim).to(_device)

                s_embed_seq_tensor_r = torch.zeros(len(s_len_non_zero), self.seq_len, 3 * self.h_dim).to(_device)
    ...
    def predict(self, s_history, s, r, ent_embeds, rel_embeds, graph_dict, global_emb, reverse):
        ...
        inp = torch.zeros(len(s_hist), 4 * self.h_dim).to(_device)
        ...
        inp_r = torch.zeros(len(s_hist), 3 * self.h_dim).to(_device)
...
class MeanAggregator(nn.Module):
    ...
    def forward(self, s_hist, s, r, ent_embeds, rel_embeds):
        ...
        embeds_mean = embeds_sum / torch.Tensor(len_s).to(_device).view(-1, 1)

        if self.gcn:
            embeds_mean = self.gcn_layer(embeds_mean)
            embeds_mean = F.relu(embeds_mean)
        embeds_split = torch.split(embeds_mean, s_len_non_zero.tolist())
        s_embed_seq_tensor = torch.zeros(len(s_len_non_zero), self.seq_len, 2 * self.h_dim).to(_device)
    ...
    def predict(self, s_history, s, r, ent_embeds, rel_embeds):
        inp = torch.zeros(len(s_history), 2 * self.h_dim).to(_device)
...
class AttnAggregator(nn.Module):
    ...
    def forward(self, s_hist, s, r, ent_embeds, rel_embeds):
        ...
        s_embed_seq_tensor = torch.zeros(len(s_len_non_zero), self.seq_len, 3 * self.h_dim).to(_device)
    ...
    def predict(self, s_history, s, r, ent_embeds, rel_embeds):
        inp = torch.zeros(len(s_history), 3 * self.h_dim).to(_device)
